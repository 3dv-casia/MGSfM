...
image_t RigMaximumSpanningTree(
  ...
  std::unordered_map<image_t, int> image_id_to_idx;
  image_id_to_idx.reserve(ref_image_ids.size());
  std::unordered_map<int, image_t> idx_to_image_id;
  idx_to_image_id.reserve(ref_image_ids.size());

  for (auto& image_id : ref_image_ids) {
  ...
  double max_weight = 0;
  for (const auto& pair_info : median_rig_pairs) {
    max_weight =
        std::max(max_weight, static_cast<double>(pair_info.num_inliers));
  }
  ...
  edge_desc e;
  for (const auto& pair_info : median_rig_pairs) {
    int idx1 = image_id_to_idx[pair_info.ref_image_id1];
    int idx2 = image_id_to_idx[pair_info.ref_image_id2];

    // Set the weight to be negative, then the result would be a maximum
    // spanning tree
    e = boost::add_edge(idx1, idx2, G).first;
    weights_boost[e] = max_weight - pair_info.num_inliers;
  }
