...
OptionManager::OptionManager(bool add_project_options) {
  ...
  multi_mapper = std::make_shared<MGSfM::MultiGlobalMapperOptions>();
...
void OptionManager::AddMultiGlobalMapperOptions() {
  if (added_mapper_options_) {
    return;
  }
  added_mapper_options_ = true;
  ...
  AddAndRegisterDefaultOption("ba_iteration_num",
                              &multi_mapper->num_iteration_bundle_adjustment);
  AddAndRegisterDefaultOption("retriangulation_iteration_num",
                              &multi_mapper->num_iteration_retriangulation);
  AddAndRegisterDefaultOption("skip_preprocessing",
                              &multi_mapper->skip_preprocessing);
  AddAndRegisterDefaultOption("skip_view_graph_calibration",
                              &multi_mapper->skip_view_graph_calibration);
  AddAndRegisterDefaultOption("skip_relative_pose_estimation",
                              &multi_mapper->skip_relative_pose_estimation);
  AddAndRegisterDefaultOption("skip_rotation_averaging",
                              &multi_mapper->skip_rotation_averaging);
  AddAndRegisterDefaultOption("skip_translation_refinement",
                              &multi_mapper->skip_translation_refinement);
  AddAndRegisterDefaultOption("skip_position_averaging",
                              &multi_mapper->skip_position_averaging);
  AddAndRegisterDefaultOption("skip_finding_new_pairs",
                              &multi_mapper->skip_finding_new_pairs);
  AddAndRegisterDefaultOption("skip_bundle_adjustment",
                              &multi_mapper->skip_bundle_adjustment);
  AddAndRegisterDefaultOption("skip_retriangulation",
                              &multi_mapper->skip_retriangulation);
  AddAndRegisterDefaultOption("skip_pruning", &multi_mapper->skip_pruning);
  AddAndRegisterDefaultOption("hybrid_translation_averaging",
                              &multi_mapper->hybrid_translation_averaging);
}
void OptionManager::AddMultiGlobalMapperFullOptions() {
  AddMultiGlobalMapperOptions();

  AddMultiViewGraphCalibrationOptions();
  AddMultiRelativePoseEstimationOptions();
  AddMultiRotationEstimatorOptions();
  AddMultiTrackEstablishmentOptions();
  AddMultiGlobalPositionerOptions();
  AddMultiBundleAdjusterOptions();
  AddMultiTriangulatorOptions();
  AddMultiInlierThresholdOptions();
}
...
void OptionManager::AddMultiViewGraphCalibrationOptions() {
  if (added_view_graph_calibration_options_) {
    return;
  }
  added_view_graph_calibration_options_ = true;
  AddAndRegisterDefaultOption("ViewGraphCalib.thres_lower_ratio",
                              &multi_mapper->opt_vgcalib.thres_lower_ratio);
  AddAndRegisterDefaultOption("ViewGraphCalib.thres_higher_ratio",
                              &multi_mapper->opt_vgcalib.thres_higher_ratio);
  AddAndRegisterDefaultOption("ViewGraphCalib.thres_two_view_error",
                              &multi_mapper->opt_vgcalib.thres_two_view_error);
}
...
void OptionManager::AddMultiRelativePoseEstimationOptions() {
  if (added_relative_pose_options_) {
    return;
  }
  added_relative_pose_options_ = true;
  AddAndRegisterDefaultOption(
      "RelPoseEstimation.max_epipolar_error",
      &multi_mapper->opt_relpose.ransac_options.max_epipolar_error);
}
...
void OptionManager::AddMultiRotationEstimatorOptions() {
  if (added_rotation_averaging_options_) {
    return;
  }
  added_rotation_averaging_options_ = true;
  AddAndRegisterDefaultOption("SingleRotationAveraging.irls_sigma",
                              &multi_mapper->opt_ra.irls_loss_parameter_sigma);
  AddAndRegisterDefaultOption("MultipleRotationAveraging.irls_sigma",
                              &multi_mapper->opt_mre.irls_loss_parameter_sigma);
}
...
void OptionManager::AddMultiTrackEstablishmentOptions() {
  if (added_track_establishment_options_) {
    return;
  }
  added_track_establishment_options_ = true;
  AddAndRegisterDefaultOption("TrackEstablishment.min_num_tracks_per_view",
                              &multi_mapper->opt_track.min_num_tracks_per_view);
  AddAndRegisterDefaultOption("TrackEstablishment.min_num_view_per_track",
                              &multi_mapper->opt_track.min_num_view_per_track);
  AddAndRegisterDefaultOption("TrackEstablishment.max_num_view_per_track",
                              &multi_mapper->opt_track.max_num_view_per_track);
  AddAndRegisterDefaultOption("TrackEstablishment.max_num_tracks",
                              &multi_mapper->opt_track.max_num_tracks);
}
...
void OptionManager::AddMultiGlobalPositionerOptions() {
  if (added_global_positioning_options_) {
    return;
  }
  added_global_positioning_options_ = true;
  AddAndRegisterDefaultOption("MultiplePositionRefinement.use_gpu",
                              &multi_mapper->opt_mpr.use_gpu);
  AddAndRegisterDefaultOption("MultiplePositionRefinement.gpu_index",
                              &multi_mapper->opt_mpr.gpu_index);
  AddAndRegisterDefaultOption("MultiplePositionRefinement.optimize_positions",
                              &multi_mapper->opt_mpr.optimize_positions);
  AddAndRegisterDefaultOption("MultiplePositionRefinement.optimize_points",
                              &multi_mapper->opt_mpr.optimize_points);
  AddAndRegisterDefaultOption(
      "MultiplePositionRefinement.optimize_internal_translation",
      &multi_mapper->opt_mpr.optimize_internal_translation);
  AddAndRegisterDefaultOption("MultiplePositionRefinement.angle_threshold",
                              &multi_mapper->opt_mpr.angle_thresh,
                              "angle(deg) threshold for the loss function");
  AddAndRegisterDefaultOption(
      "MultiplePositionRefinement.max_num_iterations",
      &multi_mapper->opt_mpr.solver_options.max_num_iterations);
  AddAndRegisterDefaultOption(
      "MultiplePositionRefinement.regularize_trajectory",
      &multi_mapper->opt_mpr.regularize_trajectory);
  AddAndRegisterDefaultOption("MultiplePositionRefinement.regularize_weight",
                              &multi_mapper->opt_mpr.regularize_weight);
  AddAndRegisterDefaultOption(
      "MultiplePositionRefinement.function_tolerance",
      &multi_mapper->opt_mpr.solver_options.function_tolerance);
}
...
void OptionManager::AddMultiBundleAdjusterOptions() {
  if (added_bundle_adjustment_options_) {
    return;
  }
  added_bundle_adjustment_options_ = true;
  AddAndRegisterDefaultOption("BundleAdjustment.use_gpu",
                              &multi_mapper->opt_ba.use_gpu);
  AddAndRegisterDefaultOption("BundleAdjustment.gpu_index",
                              &multi_mapper->opt_ba.gpu_index);
  AddAndRegisterDefaultOption("BundleAdjustment.optimize_rotations",
                              &multi_mapper->opt_ba.optimize_rotations);
  AddAndRegisterDefaultOption("BundleAdjustment.optimize_translation",
                              &multi_mapper->opt_ba.optimize_translation);
  AddAndRegisterDefaultOption("BundleAdjustment.optimize_intrinsics",
                              &multi_mapper->opt_ba.optimize_intrinsics);
  AddAndRegisterDefaultOption("BundleAdjustment.optimize_principal_point",
                              &multi_mapper->opt_ba.optimize_principal_point);
  AddAndRegisterDefaultOption("BundleAdjustment.optimize_points",
                              &multi_mapper->opt_ba.optimize_points);
  AddAndRegisterDefaultOption("BundleAdjustment.thres_loss_function",
                              &multi_mapper->opt_ba.thres_loss_function);
  AddAndRegisterDefaultOption("BundleAdjustment.optimize_internal_rotation",
                              &multi_mapper->opt_ba.optimize_internal_rotation);
  AddAndRegisterDefaultOption(
      "BundleAdjustment.optimize_internal_translation",
      &multi_mapper->opt_ba.optimize_internal_translation);
  AddAndRegisterDefaultOption(
      "BundleAdjustment.max_num_iterations",
      &multi_mapper->opt_ba.solver_options.max_num_iterations);
}
...
void OptionManager::AddMultiTriangulatorOptions() {
  if (added_triangulation_options_) {
    return;
  }
  added_triangulation_options_ = true;
  AddAndRegisterDefaultOption(
      "Triangulation.complete_max_reproj_error",
      &multi_mapper->opt_triangulator.tri_complete_max_reproj_error);
  AddAndRegisterDefaultOption(
      "Triangulation.merge_max_reproj_error",
      &multi_mapper->opt_triangulator.tri_merge_max_reproj_error);
  AddAndRegisterDefaultOption("Triangulation.min_angle",
                              &multi_mapper->opt_triangulator.tri_min_angle);
  AddAndRegisterDefaultOption("Triangulation.min_num_matches",
                              &multi_mapper->opt_triangulator.min_num_matches);
}
...
void OptionManager::AddMultiInlierThresholdOptions() {
  if (added_inliers_options_) {
    return;
  }
  added_inliers_options_ = true;
  AddAndRegisterDefaultOption("Thresholds.max_angle_error",
                              &multi_mapper->inlier_thresholds.max_angle_error);
  AddAndRegisterDefaultOption(
      "Thresholds.max_reprojection_error",
      &multi_mapper->inlier_thresholds.max_reprojection_error);
  AddAndRegisterDefaultOption(
      "Thresholds.min_triangulation_angle",
      &multi_mapper->inlier_thresholds.min_triangulation_angle);
  AddAndRegisterDefaultOption(
      "Thresholds.max_epipolar_error_E",
      &multi_mapper->inlier_thresholds.max_epipolar_error_E);
  AddAndRegisterDefaultOption(
      "Thresholds.max_epipolar_error_F",
      &multi_mapper->inlier_thresholds.max_epipolar_error_F);
  AddAndRegisterDefaultOption(
      "Thresholds.max_epipolar_error_H",
      &multi_mapper->inlier_thresholds.max_epipolar_error_H);
  AddAndRegisterDefaultOption("Thresholds.min_inlier_num",
                              &multi_mapper->inlier_thresholds.min_inlier_num);
  AddAndRegisterDefaultOption(
      "Thresholds.min_inlier_ratio",
      &multi_mapper->inlier_thresholds.min_inlier_ratio);
  AddAndRegisterDefaultOption(
      "Thresholds.max_rotation_error",
      &multi_mapper->inlier_thresholds.max_rotation_error);
}
...
void OptionManager::ResetOptions(const bool reset_paths) {
  ...
  *multi_mapper = MGSfM::MultiGlobalMapperOptions();
