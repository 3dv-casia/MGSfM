...
size_t TrackEngine::EstablishFullTracksV2(
    std::unordered_map<track_t, Track>& tracks) {
  tracks.clear();
  uf_.Clear();
...
void TrackEngine::BlindConcatenation() {
  ...
  const uint32_t print_counter_pair =
      std::max<uint32_t>(view_graph_.image_pairs.size() / 10, 1);
  for (const auto& pair : view_graph_.image_pairs) {
    if ((counter + 1) % print_counter_pair == 0 ||
...
void TrackEngine::TrackCollection(std::unordered_map<track_t, Track>& tracks) {
  ...
  for (auto& [track_id, correspondence_set] : track_map) {
    ...
    if ((counter + 1) % print_counter_track == 0 ||
        counter == track_map.size() - 1) {
...
void TrackEngine::TrackCollectionV2(
    std::unordered_map<track_t, Track>& tracks) {
  std::unordered_map<uint64_t, std::unordered_set<uint64_t>> track_map;
  ...
  for (auto& [track_id, correspondence_set] : track_map) {
    ...
    for (auto point_global_id : correspondence_set) {
      ...
      image_id_set[image_id].push_back(point);
      track.observations.emplace_back(image_id, feature_id);
    }
  }
  std::cout << std::endl;
  LOG(INFO) << "Established tracks num: " << tracks.size();
}
