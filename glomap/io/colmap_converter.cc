...
void ConvertGlomapToColmap(const std::unordered_map<camera_t, Camera>& cameras,
  ...
  for (const auto& [camera_id, camera] : cameras) {
    if (static_cast<int>(camera.model_id) == -1 ||
        camera_id == colmap::kInvalidCameraId)
      continue;
...
void ConvertDatabaseToGlomap(const colmap::Database& database,
  ...
  image_t counter = 0;
  const uint32_t print_counter =
      std::max<uint32_t>(images_colmap.size() / 10, 1);
  for (auto& image : images_colmap) {
    if ((counter + 1) % print_counter == 0 ||
        counter == images_colmap.size() - 1)
    ...
    if (prior.IsValid()) {
      const Rigid3d world_from_cam_prior(Eigen::Quaterniond::Identity(),
                                         prior.position);
  ...
  const uint32_t print_id = std::max<uint32_t>(all_matches.size() / 10, 1);
  for (size_t match_idx = 0; match_idx < all_matches.size(); match_idx++) {
    if ((match_idx + 1) % print_id == 0 || match_idx == all_matches.size() - 1)
