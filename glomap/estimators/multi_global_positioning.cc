...
MultiGlobalPositioner::MultiGlobalPositioner(
    const MultiGlobalPositionerOptions& options)
    : options_(options) {
  random_generator_.seed(options_.seed);
}

bool MultiGlobalPositioner::Solve(
  ...
  if (view_graph.image_pairs.empty() &&
      options_.constraint_type != MultiGlobalPositionerOptions::ONLY_POINTS) {
    LOG(ERROR) << "Number of image_pairs = " << view_graph.image_pairs.size();
    return false;
  }
  if (tracks.empty() &&
      options_.constraint_type != MultiGlobalPositionerOptions::ONLY_CAMERAS) {
  ...
  if (options_.constraint_type != MultiGlobalPositionerOptions::ONLY_POINTS) {
    AddCameraToCameraConstraints(
        view_graph, images, rel_from_refs, image_ref_id, rig_ref_images);
  }

  // Add the point to camera constraints to the problem.
  if (options_.constraint_type != MultiGlobalPositionerOptions::ONLY_CAMERAS) {
    AddPointToCameraConstraints(
        cameras, images, tracks, rel_from_refs, image_ref_id, rig_ref_images);
  }
  ...
  for (auto& [image_id, image] : images) {
    ...
    image_t ref_image_id = rig_ref_images.at(image_ref_id.at(image_id));
...
void MultiGlobalPositioner::SetupProblem(
    const ViewGraph& view_graph,
...
void MultiGlobalPositioner::InitializeRandomPositions(
    const ViewGraph& view_graph,
    std::unordered_map<image_t, Image>& images,
    std::unordered_map<track_t, Track>& tracks) {
...
void MultiGlobalPositioner::AddCameraToCameraConstraints(
    const ViewGraph& view_graph,
    std::unordered_map<image_t, Image>& images,
    std::unordered_map<camera_t, Rigid3d>& rel_from_refs,
    const std::unordered_map<image_t, image_t>& image_ref_id,
    const std::unordered_map<image_t, image_t>& rig_ref_images) {
  for (const auto& [pair_id, image_pair] : view_graph.image_pairs) {
    if (image_pair.is_valid == false) continue;

    const image_t image_id1 = image_pair.image_id1;
    const image_t image_id2 = image_pair.image_id2;
    if (image_ref_id.at(image_id1) == image_ref_id.at(image_id2)) {
      continue;
    }
    ...
    image_t ref_image_id1 =
        rig_ref_images.at(image_ref_id.at(image_pair.image_id1));
    image_t ref_image_id2 =
        rig_ref_images.at(image_ref_id.at(image_pair.image_id2));
    ...
    if (!options_.generate_scales && !options_.generate_random_positions) {
      const Eigen::Vector3d trans_calc =
          images.at(image_pair.image_id2).cam_from_world.translation -
          images.at(image_pair.image_id1).cam_from_world.translation;
      scale = std::max(
          1e-5, translation_obs.dot(trans_calc) / trans_calc.squaredNorm());
    }

    if (camera_id1 != camera_id2) {
      ceres::CostFunction* cost_function = C2CBATADirectionError::Create(
          translation_obs,
          images.at(image_pair.image_id1).cam_from_world.rotation.inverse(),
          images.at(image_pair.image_id2).cam_from_world.rotation.inverse());
      ...
      ceres::CostFunction* cost_function = SC2CBATADirectionError::Create(
          translation_obs,
          images.at(image_pair.image_id1).cam_from_world.rotation.inverse(),
          images.at(image_pair.image_id2).cam_from_world.rotation.inverse());
...
void MultiGlobalPositioner::AddPointToCameraConstraints(
    std::unordered_map<camera_t, Camera>& cameras,
    std::unordered_map<image_t, Image>& images,
    std::unordered_map<track_t, Track>& tracks,
    std::unordered_map<camera_t, Rigid3d>& rel_from_refs,
  ...
  if (num_cam_to_cam > 0 &&
      options_.constraint_type ==
          MultiGlobalPositionerOptions::POINTS_AND_CAMERAS_BALANCED) {
...
void MultiGlobalPositioner::AddTrackToProblem(
    track_t track_id,
    std::unordered_map<camera_t, Camera>& cameras,
    std::unordered_map<image_t, Image>& images,
    std::unordered_map<track_t, Track>& tracks,
  ...
  for (const auto& observation : tracks[track_id].observations) {
    ...
    image_t ref_image_id =
        rig_ref_images.at(image_ref_id.at(observation.first));
    ...
    // Eigen::Vector3d translation_b = image.cam_from_world.rotation.inverse() *
    //                                 rel_from_refs.at(camera_id).translation;

    // ceres::CostFunction* cost_function =
    //     DeMultiBATAPairwiseDirectionError::Create(translation_obs,
    //                                               translation_b);

    ceres::CostFunction* cost_function = C2PBATADirectionError::Create(
    ...
    if (cameras[camera_id].has_prior_focal_length) {
      problem_->AddResidualBlock(
          cost_function,
...
void MultiGlobalPositioner::AddCamerasAndPointsToParameterGroups(
    std::unordered_map<image_t, Image>& images,
    std::unordered_map<track_t, Track>& tracks,
    std::unordered_map<camera_t, Rigid3d>& rel_from_refs) {
  ...
  if (tracks.size() > 0) {
    for (auto& [track_id, track] : tracks) {
      if (problem_->HasParameterBlock(track.xyz.data()))
        parameter_ordering->AddElementToGroup(track.xyz.data(), rig_id);
    }
    rig_id++;
  }
  ...
  for (auto& [image_id, image] : images) {
    if (problem_->HasParameterBlock(image.cam_from_world.translation.data())) {
      parameter_ordering->AddElementToGroup(
          image.cam_from_world.translation.data(), rig_id);
    }
  }
...
void MultiGlobalPositioner::ParameterizeVariables(
    std::unordered_map<image_t, Image>& images,
    std::unordered_map<track_t, Track>& tracks,
    std::unordered_map<camera_t, Rigid3d>& rel_from_refs) {
...
void MultiGlobalPositioner::ConvertResults(
    std::unordered_map<image_t, Image>& images) {
